...
def main():
    ...
    melt_df.rename(columns={"Protein ID":"meltome-id",
                            ...
                            "R2 of curve fit":"meltome-fit-R2"}, inplace=True)

    melt_df["UniProtKB-AC"] = melt_df["meltome-id"].str.split("_").str[0]

    nodes_df = nodes_df.merge(melt_df, on="UniProtKB-AC", how="left")

    output_path = f"{args.output_dir}/{args.output_prefix}-{args.organism_tag}-nodes-centrality-seqs-DeepTMHMM-SignalP-UniProt-IDRs-albatross-cider-GhoshDill-Cagiada-Rosetta-FoldX-halflife-expr-speed-PTMGPT2-LiPMS-entanglement-chaperones-oligomers-domains-essential-Y2H-meltome.pkl"
