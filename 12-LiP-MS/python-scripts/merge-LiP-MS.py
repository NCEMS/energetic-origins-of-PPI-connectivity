...
def main():
    ...
    nodes_df = nodes_df.merge(
        refold, how="left", right_on="Protein ID", left_on="UniProtKB-AC"
    )
    nodes_df = nodes_df.drop("Protein ID", axis=1)

    # merge in the heatshock information
    nodes_df = nodes_df.merge(
        heatshock, how="left", right_on="Protein ID", left_on="UniProtKB-AC"
    )
    nodes_df = nodes_df.drop("Protein ID", axis=1)

    # merge in the recovery information
    nodes_df = nodes_df.merge(
        recovery, how="left", right_on="Protein ID", left_on="UniProtKB-AC"
    )
    nodes_df = nodes_df.drop("Protein ID", axis=1)
