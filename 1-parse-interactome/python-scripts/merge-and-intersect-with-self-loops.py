...
def canonicalize_undirected_edges(edges_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Self-interactions are intentionally retained. That is, an edge with
    source == target is a valid edge and is not filtered out.

    Rows with missing source or target values are removed before
    canonicalization to avoid converting A-NA into a false A-A self-edge.
    """
    ...
    # This removes exact duplicate edges only.
    # It does not remove self-interactions.
...
def get_nodes(edges_df: pd.DataFrame) -> set:
    ...
    Nodes participating only in self-interactions are retained because
    self-interaction edges are retained in edges_df.
    """
...
def filter_edges_to_node_set(edges_df: pd.DataFrame, node_set: set) -> pd.DataFrame:
    ...
    Self-interactions are intentionally retained when the self-interacting
    node is present in node_set.
    """
...
def count_self_interactions(edges_df: pd.DataFrame) -> int:
    """
    Count edges where source == target.
    """
    return int((edges_df["source"] == edges_df["target"]).sum())
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from two protein edge files. Self-interactions are retained."
        )
    )
    ...
    # Self-interactions are retained when the self-interacting node is in the
    ...
    print(f"Self-interactions in file1:               {count_self_interactions(edges1):,}")
    print(f"Self-interactions in file2:               {count_self_interactions(edges2):,}")
    print(f"Self-interactions in union:               {count_self_interactions(union_edges):,}")
    ...
    print(
        f"File1 self-interactions in intersection:  "
        f"{count_self_interactions(file1_intersection_edges):,}"
    )
    print(
        f"File2 self-interactions in intersection:  "
        f"{count_self_interactions(file2_intersection_edges):,}"
    )
