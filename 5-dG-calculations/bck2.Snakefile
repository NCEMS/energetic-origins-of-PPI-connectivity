...
# helper function to ID AlphaFold2 predictions that need to be run
def get_alphafold_jobs_tagged():
    base_dir = os.path.join(config["dG_predictions"]["output_dir"], "alphafold")
    all_jobs = []
    for i, path in enumerate(sorted(glob(f"{base_dir}/*"))):
        node = os.path.basename(path)
        fasta = os.path.join(path, f"{node}.fasta")
        expected_output = os.path.join(path, "result_model_1_pred_0.pdb")
        if os.path.exists(fasta) and not os.path.exists(expected_output):
            gpu_id = i % 2  # alternate between GPU 0 and GPU 1
            all_jobs.append({"node": node, "gpu": gpu_id})
    return all_jobs

def get_gpu_for_node(node):
    # Simple consistent hash to alternate GPUs
...
AF2_DOCKER        = LOCAL_CONFIG["AF2_docker_script"]
...
rule all:
    input:
        ...
        [
            f"{config['dG_predictions']['output_dir']}/alphafold/{job['node']}/result_model_1_pred_0.pdb"
            for job in get_alphafold_jobs_tagged()
        ]
