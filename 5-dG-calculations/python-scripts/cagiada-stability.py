...
def test_P78285_results_within_tolerance(f_path):
    ...
    # check assertions
    assert (
        abs(ref_likelihood_sum - new_likelihood_sum) < tol
    ), "Reference and calculated likelihood sums do not match"
    assert (
...
def score_variants(sequence, token_probs, alphabet):
    ...
    for i, n in enumerate(sequence):
        ...
        score_pos = [
            masked_absolute(alphabetAA_D_L[j], i, token_probs, alphabet)
            for j in range(1, 21)
        ]
...
def predict_dG(cagiada_info, output_dir, model, alphabet, create_file=False):
    ...
    prob_tokens = run_model(
        coords_structure,
        sequence_structure,
        model,
        alphabet,
        chain_target=cagiada_info.chainID,
    )
    ...
    if create_file:
        ...
        output_file = os.path.join(
            output_dir, cagiada_info.output_name + "-cagiada-dG.csv"
        )
        output_df = pd.DataFrame(
            {
                "Residue": aa_list + ["dG_IF", "dG_kcalmol"],
                "score": wt_scores + [dg_IF, dg_kcalmol],
            }
        )
