...
rule mark_af2_complete:
    input:
        targets = f"{OUTPUT_DIR}/{OUTPUT_PREFIX}-{ORGANISM_TAG}-alphafold-targets.txt"
    output:
        touch(f"{OUTPUT_DIR}/all-predictions-done.txt")
    run:
        nodes = read_targets_file(input.targets)
        for node in nodes:
            expected = f"{OUTPUT_DIR}/alphafold/{node}/result_model_1_pred_0.pdb"
            if not os.path.exists(expected):
                raise ValueError(f"Missing AlphaFold output: {expected}")
