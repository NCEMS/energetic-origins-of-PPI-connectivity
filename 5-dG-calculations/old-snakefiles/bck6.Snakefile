...
# === Load AlphaFold2 targets ===
def get_af2_targets():
    target_file = OUTPUT_DIR / f"{OUTPUT_PREFIX}-{ORGANISM_TAG}-alphafold-targets.txt"
...
rule all:
    input:
        expand(
            f"{LOCAL_CONFIG['output_dir']}/alphafold/{{node}}/result_model_1_pred_0.pdb",
            node=get_af2_targets()
        ),
        OUTPUT_DIR / f"{OUTPUT_PREFIX}-{ORGANISM_TAG}-nodes-centrality-seqs-DeepTMHMM-SignalP-UniProt-IDRs-albatross-cider-GhoshDill-Cagiada.pkl"

rule prepare_structures:
    input:
        nodes = DATA_DIR1 / f"{OUTPUT_PREFIX}-{ORGANISM_TAG}-nodes-centrality-seqs-DeepTMHMM-SignalP-UniProt-IDRs-albatross-cider.pkl"
    output:
        struct_pkl = OUTPUT_DIR / f"{OUTPUT_PREFIX}-{ORGANISM_TAG}-temp.pkl"
    conda:
        "env/prepare-structures.yml"
    params:
        input_dir     = STRUCTURE_DIR,
...
rule run_alphafold2:
    input:
        fasta = lambda wildcards: f"{LOCAL_CONFIG['output_dir']}/alphafold/{wildcards.node}/{wildcards.node}.fasta",
        struct_pkl = OUTPUT_DIR / f"{OUTPUT_PREFIX}-{ORGANISM_TAG}-temp.pkl"
    output:
        pdb = f"{LOCAL_CONFIG['output_dir']}/alphafold/{{node}}/result_model_1_pred_0.pdb"
    params:
...
rule ghosh_dill_stability:
    input:
        nodes = OUTPUT_DIR / f"{OUTPUT_PREFIX}-{ORGANISM_TAG}-temp.pkl"
    output:
        nodes = OUTPUT_DIR / f"{OUTPUT_PREFIX}-{ORGANISM_TAG}-nodes-centrality-seqs-DeepTMHMM-SignalP-UniProt-IDRs-albatross-cider-GhoshDill.pkl"
    conda:
