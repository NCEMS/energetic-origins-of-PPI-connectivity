...
def get_alphafold_outputs():
    checkpoint_output = checkpoints.prepare_structures_checkpoint.get().output.struct_pkl
    checkpoint_dir = os.path.dirname(checkpoint_output)
    targets = discover_alphafold_targets(checkpoint_dir)
    return expand(
...
rule all:
    input:
        lambda wildcards: get_alphafold_outputs(),
