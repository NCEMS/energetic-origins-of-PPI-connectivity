...
class CentralityCosDist:
    ...
    def Cosine_similarity(self, method="cosine"):
        ...
        df = pd.DataFrame(
            Distance, columns=Centrality_df.index, index=Centrality_df.index
        )
        # print(df)
        # self.Test = df
        ...
        df_avg = df.copy()
        df_avg["_Average_distance"] = df_avg.mean(
        ...
        df_avg["_Percentile_rank"] = df_avg["_Average_distance"].rank(
            pct=True, ascending=False
        )
        df_avg = df_avg[list(df_avg)[-4:] + list(df_avg)[:-4]]
        df_return = pd.concat([Centrality_df, df_avg], axis=1).reindex(
            Centrality_df.index
        )
